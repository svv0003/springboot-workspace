...
사용자가 웹 어플리케이션에 접속하면 서버는 고유한 세선을 할당하고,
클라이언트는 그 세션을 식별하는 세션 ID를 서버에 전송한다.

세션 데이터는 서버에 저장되고, 클라이언트 측에는 세션 ID만 저장된다.
보통 쿠키에 세션 ID가 저장된다.

쿠키 (Cookie)
쿠키는 사용자 브라우저에 저장되고, 서버는 쿠키 데이터를 읽고 쓸 수 있다.
중요한 데이터를 저장하기에는 클라이언트에 저장이기 때문에 보안상 위험하므로
광고, 마케팅, 테마와 같은 동의 여부 정도만 저장한다.

로컬 스토리지 (Local Storage)
HTML에서 도입된 클라이언트 측 데이터 저장 방식
쿠키와 달리 서버에 데이터를 저장하지 않고, 오로지 클라이언트 브라우저에만 데이터를 저장한다.
저장된 데이터는 유효기간이 없기 때문에 사용자가 직접 삭제하지 않으면 컴퓨터를 종료해도 데이터가 영원히 남아있다.
장바구니 상태나 단순히 저장할 상태 정도만 저장
 */
