...
public class UserService {
    ...
    public void insertUser(User user) {
        ...
        /**
         * 비밀번호 암호화 같은 복합 작업을 진행하는 공간
         * 프로필 사진을 폴더에 저장하고, 폴더 경로도 DB에 저장 가능하다.
         */
        String 유저작성패스워드 = user.getPassword();
        String 암호화된패스워드 = bCryptPasswordEncoder.encode(유저작성패스워드);
        // 암호화처리 된 패스워드로 교체하여 user DB에 저장하도록 설정한다.
        user.setPassword(암호화된패스워드);

        /**
         * user.setRole("USER");
         * html에서 클라이언트가 작성 설정하는 것이 아니라
         * 개발자의 회사에서 클라이언트 정보를 강제적으로
         * 기본값 설정할 때 추가 로직 작성 가능하다.
         */
